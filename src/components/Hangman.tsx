...
interface HangmanProps {
    words: {palabra:string,pista: string}[];
}
...
const Hangman = ({ words }: HangmanProps) => { 
    ...
    const displayWord = selectedWords.palabra.split('').map((letter) => {
    ...
    const handleGuess = (letter: string) => {
        if (!guessdLetters.includes(letter)) {
            ...
            if (!selectedWords.palabra.includes(letter)) {
    ...
    return (
        ...
        <div>
            ...
            {displayWord.join('') === selectedWords.palabra && (
